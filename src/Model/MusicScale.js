...
//#region Public Functions
  ...
  init(){
    ...
    this.ExtendedScaleToneNames = {...this.BuildExtendedScaleToneNames(this.ExtendedScaleSteps,this.RootNote.note,this.Notations,this.Name)};
...
//#region Naming Functions
  ...
  BuildExtendedScaleToneNames(semiToneSteps, rootNoteName, notation,scaleName){
    ...
    notation.forEach(whichNotation => {
      ...
      case "Scale_Steps":
          if(this.Name == "Chromatic"){
            ...
            let relative = this.noteIndexToScaleIndex(this.Recipe.steps);
            theScale["Scale_Steps"] = semiToneSteps.map((step,index)=>this.Recipe.numbers[(index+relative)%length]);
          }
          break;
...
MakeChromatic(scaleFormula,keyName,scaleName,whichNotation) {
  ...
  let startingName = keyName;
  let offset;
  let relative = this.noteIndexToScaleIndex(scaleFormula);
  let tonenameOffset = this.noteNameToIndex(keyName)
  let startingNote =  whichNotation == "Chord_extensions" ? relative : tonenameOffset;
  ...
  let myScale = [];
    ...
    if(keyName == "F" || keyName.includes("b") ){
      try{
          myScale = myScaleFormula.map(step => TONE_NAMES[whichNotation].flats[(step+startingNote)%12])
         }
      catch {
          myScale = myScaleFormula.map( step=> "err!"+ (step+startingNote) % TONE_NAMES.English.sharps.length); // high note used to indicate error
         }
    }
    else if(this.noteNameToIndex(keyName)!= -1) {
      try{
          myScale = myScaleFormula.map(step => TONE_NAMES[whichNotation].sharps[(step+startingNote)%12])
      }
      catch {      
        myScale = myScaleFormula.map( step=> "err!"+ (step+startingNote) % TONE_NAMES.English.sharps.length); // high note used to indicate error
      }
...
makeScaleMajorMinor(scaleFormula, keyName, whichNotation) {
  ...
  for (let i = 0; i < myScaleFormula.length; i++) {
    ...
    if (
      ...
      myScale.push("err!"+(offset + i) % ALPHA_NAMES["English"].length); // high note used to indicate error
    }
  }
  // //console.log("myscale",myScale);
  return myScale;
...
makeScalePentatonicBlues(scaleFormula,keyName,scaleName,whichNotation) {
  ...
  let myScale = [];
  for (let i = 0; i < myScaleFormula.length; i++) {
    const formulaNumber = (myScaleFormula[i] + startingNote)%12;
    if (
      noteMapping.English.Sharp_Names[
        formulaNumber
      ].includes(
...
noteIndexToScaleIndex(recipe) {
  let i;
  let IndexNumber = -1;
  let firstInScale = recipe.find(e => e%12==0);
  IndexNumber = recipe.indexOf(firstInScale);
  return IndexNumber;
}
