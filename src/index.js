...
import { Octaves, Scale, Notation, BaseNote, Theme } from './components/InputComponents';
	...
	handleClickOctave = (action) => {
		switch(action) {
			...
			case 'plus': this.setState({octave: this.state.octave + 1});
				break;
			default: this.setState({octave: 3});
				break;
		}
	}
  ...
  togglePiano = () => {
	...
	render() {
        ...
        <div className={`MainMenu ${this.state.menuOpen ? "open" : ""}`}>
          <div className="closeMenu" onClick={this.toggleMenu} style={{backgroundColor: '#FFFFFF', cursor: 'pointer'}}>(x)</div>
          <div className="Menu-Row">
            <Octaves octave={this.state.octave} handleClick={this.handleClickOctave}/>
            <Scale scale={this.state.scale} handleSelect={this.handleSelectScale}/>
            <Notation notation={this.state.notation} handleSelect={this.handleSelectNotation}/>
            <CircleFifthsSVG />
          </div>
          <div className="Menu-Row">
            ...
            <Theme theme={this.state.theme} handleSelect={this.handleSelectTheme}/>
          </div>
          </div>
