...
const targetArr, activeElementsforKeyboard;
  ...
  */
  //e.preventDefault();
   ...
   if (Tone.context.state !== 'running') {
    ...
    if (e.repeat) { console.log('repeat'); return } // prevent key from firing multiple times when key pressed for a long time
    ...
    if( keycodes.includes(e.keyCode) && (mapKeyDown+1) <= this.state.scaleSteps.steps.length ) {
  ...
  handleKeyUp = (e) => {
    ...
    if( keycodes.includes(e.keyCode) && (mapKeyUp+1) <= this.state.scaleSteps.steps.length ) {
      let buttonReleased;
  ...
  componentWillUnmount() {
    ...
    for(let i=0; i<that.props.notation.length; i++){
      if( (that.props.notation[i] === 'English' || that.props.notation[i] === 'German' || that.props.notation[i] === 'Romance')){
        if( !this.props.scale.includes('Pentatonic') && !this.props.scale.includes('Blues')){
          theScale[that.props.notation[i]] = makeScaleMajorMinor(scaleSteps.steps, that.props.baseNote, that.props.notation[i] );
        } else {
          theScale[that.props.notation[i]] = makeScalePentatonicBlues(scaleSteps.steps, that.props.baseNote, this.props.scale, that.props.notation[i]);
        }
      }
    }
    
    let currentRoot = rootNote.find(obj => {return obj.note == this.props.baseNote} );
    ...
    for(let i=0; i<that.props.notation.length; i++){
      switch(that.props.notation[i]){
        ...
        case 'Romance':
        if(isKeyInScale){
           noteName.push(theScale[that.props.notation[i]][relativeCountScale]);
        }
        break;
        case 'English':
        if(isKeyInScale){
        ...
        case 'Scale Steps':
          if(isKeyInScale ){
            if(( that.props.scale !== 'Major Pentatonic' && relativeCount !==8) || (that.props.scale === 'Major Pentatonic' && relativeCount !== 7)  ){
              noteName.push(relativeCount);
            }
            relativeCount++;
            if(that.props.scale === 'Major Pentatonic' && relativeCount === 4){
                relativeCount++; // add one more, this scale doesn't have the number 4
            }
            if(that.props.scale === 'Minor Pentatonic' && (relativeCount === 2 || relativeCount === 6)){
        ...
        case 'Chord extensions':
         if(isKeyInScale){
            if(that.props.scale === 'Major Pentatonic' && relativeCountChord === 4){
                relativeCountChord++; // add one more, this scale doesn't have the number 4
            }
            if(that.props.scale === 'Minor Pentatonic' && (relativeCountChord === 2 || relativeCountChord === 6)){
                relativeCountChord++; // add one more, this scale doesn't have 2 or 6

            }
            if(relativeCountChord ==2 || relativeCountChord==4 || relativeCountChord == 6){
      ...
      return <Key
        key={index}
        index={index}
        note={`${note.note_english}${(that.props.octave+note.octaveOffset) /*+ Math.floor(index/12)*/ }`}
