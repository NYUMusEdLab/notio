...
describe("ComputerKeyboard pressing key to", () =>{
    ...
    ])("pressing [d] in Major-scale plays the major7th below the root after octave have been increased with #some_number (!!remember octave shifts at C!!)", async (root_note, octave,expected_tone, keypress,keypressCount, url) => {
        expect(SoundMaker).not.toHaveBeenCalled();
        render(
            <MemoryRouter initialEntries ={url}>
                <Route path="/shared/:sessionId" component={WholeApp} />
    ...
    ])("pressing [d] in Major-blues-scale plays the 6th below the root after octave have been increased with #some_number(!!remember octave shifts at C!!)", async (root_note, octave,expected_tone, keypress,keypressCount, url) => {
        expect(SoundMaker).not.toHaveBeenCalled();
        render(
            <MemoryRouter initialEntries ={url}>
                <Route path="/shared/:sessionId" component={WholeApp} />
